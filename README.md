...
| Thermal 해상도 | 640 × 480 |
| RGB 해상도 | 2592 × 1944 |
| `data 수집 주기` | 평상시 30초, 과열 시 1초 (config.json에서 조정 가능) |
...
| 이벤트 | 레벨 | 설명 |
|--------|------|------|
| 프로브 온도 체크 | DEBUG | `probe: running thermal check`, `probe: max_temp=36.2°C` |
| 프로브 과열 감지 | INFO | `Probe detected elevated temp (52.0°C) — triggering immediate capture` |
| 프로브 실패 | WARNING | `probe failed — backing off 5s` |
| 캡처 주기 전환 | INFO | `Capture interval changed: 30.0s → 1.0s (warning mode)` |
...
|------|-----------|------------|-----------|
| 평시 (Normal) | 30초 | 없음 | - |
| 과열 (Warning) | 1초 | **없음** (인터벌만 전환) | - |
| 경보 (Critical) | 1초 | 전송 | 로봇ID, 상태, 최고 온도, 발생 시간, 과열 범위 이미지 |
