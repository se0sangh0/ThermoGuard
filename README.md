...
├── pipeline.py             # 통합 분석 파이프라인 (배치)
├── monitor.py              # 실시간 감시 시퀀서 (캡처 + 분석 + 알림)
│
...
notepad config.json   # 또는 code config.json
...
```jsonc
{
  ...
  "roi": {
    "x1": 402, "y1": 222,           // Thermal 이미지(640x480) 기준 ROI 좌상단
    "x2": 550, "y2": 396,           // ROI 우하단
    "baseline_temp": 23.0,           // 정상 기준 온도 (°C)
...
**설정 변경이 필요할 때:**
- `config.json`을 직접 편집하거나 `python roi_selector.py`로 ROI 좌표 변경
