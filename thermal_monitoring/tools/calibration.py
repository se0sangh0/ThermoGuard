...
if len(sys.argv) >= 3:
    ...
    thermal_path = sys.argv[1]
    base = os.path.splitext(thermal_path)[0]
    rgb_path = f"{base}_visual.jpg"
else:
    # thermal_dataset 폴더에서 최신 timestamp 쌍 자동 검색
...
if thermal is None:
    ...
    print("사용법: python calibration.py [thermal.jpg] [rgb.jpg]")
    exit(1)
if rgb is None:
...
rgb_pt = cv2.perspectiveTransform(
    pt,
    H
)
