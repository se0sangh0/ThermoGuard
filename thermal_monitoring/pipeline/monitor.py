    ...
    def _evaluate_and_act(self, analysis: dict) -> None:
        ...
        try:
            ...
            if attempt_send:
                ...
                _log_monitor.debug(
                    "[DBG-MONITOR] _evaluate_and_act: attempt_send is_retry=%s status=%s base=%s",
                    is_retry, new_status.value, base,
                )
                try:
                    success = send_alarm(
                    ...
                    _log_monitor.debug(
                        "[DBG-MONITOR] _evaluate_and_act: send_alarm returned success=%s", success,
                    )
                    if success:
